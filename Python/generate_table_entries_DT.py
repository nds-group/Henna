...
second_stage_models = ['2nd_stage_Plugs_model.sav', '2nd_stage_Sensors_model.sav','2nd_stage_Video_model.sav',\
    '2nd_stage_Appliances_model.sav', '2nd_stage_Computers_model.sav'] # list of trained DT models to map
...
with open("te_unsw_s2.py", "w") as entries_file: # filename to store table entries

    print("p4 = bfrt.henna.pipe\n", file=entries_file) # replace "henna" with p4 filename if changed

    for mod in range(len(second_stage_models)):
        ## import and get entries from trained models
        clf = pd.read_pickle(second_stage_models[0])
    ...
    # Get entries for feature tables
        ...
        for fea in range(0,len(feature_names)):
            tbl_name = "tbl_s2_g" + str(mod)+"_f" + str(fea)
            fea_name = "s2_g" + str(mod)+"_f" + str(fea)
            ...
            for ran, cods0 in zip(Ranges, Codes.iloc[:,0]):
                if(ran == Ranges[len(Ranges)-1]):
                    ...
                    ", "+ fea_name+"_end="+str(65535)+", code0="+str(cods0) + ")", file = entries_file)
                else:
                    print(tbl_name+".add_with_SetCode_"+fea_name+"("+fea_name+"_start="+str(ran.split(",")[0])+ \
                    ", "+ fea_name+"_end="+str(ran.split(",")[1])+", code0="+str(cods0) + ")", file = entries_file)
        ...
        for cod, mas, cla, cer in zip(Final_Codes, Final_Masks, Classe, Certain):
            print(code_tbl_name+".add_with_SetClass_s2_g"+str(mod)+"(cw_s2_g"+str(mod)+"=", cod, ", cw_s2_g"+str(mod)+\
